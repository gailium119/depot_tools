...
def _get_remoteexec_default():
    root_dir = gclient_paths.GetPrimarySolutionPath()
    if not root_dir:
        return None
    default_file = os.path.join(root_dir, "build/toolchain/remoteexec_default")

    # TODO(crbug.com/397994249): default empty once
    # build/toolchain/remoteexec_default is provided.
    values = {
    ...
    if gn_helper.exists(output_dir):
        ...
        if use_reclient is None:
            if os.path.exists(os.path.join(output_dir, ".reproxy_tmp")):
                ...
                values = _get_remoteexec_default()
                if use_siso:
