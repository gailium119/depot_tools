...
def _get_use_reclient_value(output_dir):
  ...
  if not root_dir:
    return None
  script_path = os.path.join(root_dir, "build/toolchain/use_reclient_value.py")
  ...
  try:
    ...
    raise RuntimeError(
        'Could not call method "use_reclient_value" in {}"'.format(script_path)
    )
  if not isinstance(r, bool):
    raise TypeError(
        'Method "use_reclient_defualt" in "{}" returns invalid result. Expected'
        ' bool, got "{}" (type "{}")'.format(script_path, r, type(r))
    )
  return r
...
def _get_use_siso_default(output_dir):
  ...
  if not isinstance(r, bool):
    raise TypeError(
        'Method "use_siso_default" in "{}" returns invalid result. Expected'
        ' bool, got "{}" (type "{}")'.format(script_path, r, type(r))
    )
  return r
...
def _main_inner(input_args, build_id, should_collect_logs=False):
  ...
  if gn_helper.exists(output_dir):
    ...
    if os.path.exists(siso_marker):
      print(
          "Run gn clean before switching from siso to ninja in %s" % output_dir,
          file=sys.stderr,
      )
      return 1
