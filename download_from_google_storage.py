...
def main(args):
    ...
    parser.add_option('-n',
                      '--no_auth',
                      action='store_true',
                      help='DEPRECATED: Skip auth checking.  Use if it\'s '
                      'known that the target bucket is a public bucket.')
    parser.add_option('-p',
                      '--platform',
    ...
    if 'NO_AUTH_BOTO_CONFIG' in os.environ:
        print(
            ...
            'has no effect. gsutil.py will automatically uses any [GSUtil] or ',
            '[Boto] settings in your configured .boto configuration file, ',
            'regardless of whether you\'re logged in with luci-auth.')
