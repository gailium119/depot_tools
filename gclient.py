...
class GcsDependency(Dependency):
    ...
    def _MaybeExtractArtifact(self):
        ...
        if tarfile.is_tarfile(self.artifact_output_file):
            ...
            with zipfile.ZipFile(self.artifact_output_file) as zip:
                zip_content_file = os.path.join(
                        self.output_dir, f'.{self.file_prefix}_content_names')
                self.WriteToFile(json.dumps(zip.namelist()), zip_content_file)

                zip.extractall(path=self.output_dir)
