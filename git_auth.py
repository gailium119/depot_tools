...
class ConfigWizard(object):
    ...
    def run(self, remote_url: str):
        with self._handle_config_errors():
            ...
            if _is_gerrit_url(remote_url):
                ...
                self._println(f'so we will check your Git configuration for {remote_url}')
                self._println()
    ...
    def _run_outside_repo(self) -> None:
        ...
        if not self._ui.read_yn('Set up commonly used Gerrit hosts?', default=True):
            return
    ...
    def _check_use_sso(self, parts: urllib.parse.SplitResult, email: str) -> bool:
    ...
    def _print_manual_instructions(self) -> None:
        self._println()
        self._println('Follow this for instructions on manually configuring Gerrit authentication:')
        self._println('https://commondatastorage.googleapis.com/chrome-infra-docs/flat/depot_tools/docs/html/depot_tools_gerrit_auth.html')
...
def _check_choice(choices: Collection[str]) -> _InputChecker:
    ...
    def func(ui: _UserInterface, input: str) -> bool:
        if input in choices:
            return True
...
def _is_gerrit_url(url: str) -> bool:
    ...
    if parts.netloc.endswith('.googlesource.com') or parts.netloc.endswith('.git.corp.google.com'):
        return True
    return False
...
def _creds_helper_key(parts: urllib.parse.SplitResult) -> str:
    ...
    return f'credential.{_url_base_url(parts)}.helper'
...
def _https_rewrite_key(parts: urllib.parse.SplitResult) -> str:
    ...
    return f'url.https://{_url_shortname(parts)}/.insteadOf'


def _url_review_host(parts: urllib.parse.SplitResult) -> str:
    """Format split URL as Gerrit review host.
...
def _url_shortname(parts: urllib.parse.SplitResult) -> str:
    """Format split URL as Gerrit host shortname.
...
def _url_base_url(parts: urllib.parse.SplitResult) -> str:
    """Format split URL as Gerrit host base URL.
