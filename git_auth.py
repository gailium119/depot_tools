...
class ConfigWizard(object):
    ...
    def run_for_host(self, host: str) -> None:
        with self._handle_config_errors():
            self._run_for_host(host)
    ...
    def _check_host(self, host: str) -> tuple[str, bool]:
        ...
    ...
    def _check_global_email(self) -> str:
        ...
        if not name:
            ...
            scm.GIT.SetConfig(os.getcwd(), 'user.name', name, scope='global')
        email = self._ui.read_line('Enter your email', check=_check_nonempty)
        scm.GIT.SetConfig(os.getcwd(), 'user.email', name, scope='global')
        return email
