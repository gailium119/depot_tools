...
# TODO(b/386840832): remove the below imports
from cl_format import DieWithError
from cl_format import RunCommand
from cl_format import RunGit
from cl_format import RunGitDiffCmd
...
def CMDformat(parser, args):
    ...
    formatters = [
        (GN_EXTS, cl_format._RunGnFormat),
        (['.xml'], cl_format._RunMetricsXMLFormat),
    ]
    if not opts.no_java:
        formatters += [(['.java'], cl_format._RunGoogleJavaFormat)]
    if opts.clang_format:
        formatters += [(clang_exts, cl_format._RunClangFormatDiff)]
    if opts.use_rust_fmt:
        formatters += [(['.rs'], cl_format._RunRustFmt)]
    if opts.use_swift_format:
        formatters += [(['.swift'], cl_format._RunSwiftFormat)]
    if opts.python is not False:
        formatters += [(['.py'], cl_format._RunYapf)]
    if opts.mojom:
        formatters += [(['.mojom'], cl_format._RunMojomFormat)]
    if opts.lucicfg:
        formatters += [(['.star'], cl_format._RunLUCICfgFormat)]
