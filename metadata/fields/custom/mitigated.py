...
def validate_cves(cves: str) -> Tuple[List[str], List[str]]:
=======
_VULN_ID_PATTERN = re.compile(rf"^({_PREFIX_PATTERN})-[a-zA-Z0-9]{{4}}-[a-zA-Z0-9:-]+$")
...
<<<<<<< PATCH SET (f44d59 [dependency_metadata] Allow descriptions for CVEs)
        if bool(VULN_ID_PATTERN_WITH_ANCHORS.match(cve_stripped)):
=======
...
class MitigatedField(field_types.MultiLineTextField):
    """Field for comma-separated vulnerability IDs. Supports multiline values."""
