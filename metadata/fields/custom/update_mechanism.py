...
class UpdateMechanismField(field_types.SingleLineTextField):
    ...
    def validate(self, value: str) -> Optional[vr.ValidationResult]:
        ...
        if bug_link:
            ...
            if bug_link != canonical_bug_link or not bug_num.isdigit():
                # If it ends in a '/number', then provide a copy pastable correct value.
                if bug_num.isdigit():
                    validation_result_type = vr.ValidationError
                    # If they've only missed 'http(s)://', or '.com', then just warn.
                    if BUG_LINK_REGEX.match(bug_link):
                        validation_result_type = vr.ValidationWarning
