...
class DependencyValidationTest(unittest.TestCase):
    ...
    def test_is_license_allowlisted(self):
        ...
        self.assertTrue(dependency.is_license_allowlisted("MIT", False))
        self.assertTrue(dependency.is_license_allowlisted("MIT, Apache-2.0", False))
        self.assertTrue(dependency.is_license_allowlisted("MPL-2.0", True))
        self.assertFalse(dependency.is_license_allowlisted("InvalidLicense", False))
        self.assertFalse(dependency.is_license_allowlisted("MIT, InvalidLicense", False))
        self.assertFalse(dependency.is_license_allowlisted("", False))
        self.assertFalse(dependency.is_license_allowlisted("MPL-2.0", False))


    def test_only_opensource_licenses(self):
        """Test that only open source licenses are returned."""
        dependency = dm.DependencyMetadata()
        self.assertEqual(dependency.only_opensource_licenses(""), [])
        self.assertEqual(dependency.only_opensource_licenses("MIT"), [])
        self.assertEqual(dependency.only_opensource_licenses("MPL-2.0"), ["MPL-2.0"])
        result = dependency.only_opensource_licenses("MIT, MPL-2.0")
        self.assertEqual(result, ["MPL-2.0"])
        result = dependency.only_opensource_licenses("MPL-2.0, APSL-2.0")
        self.assertEqual(set(result), {"MPL-2.0", "APSL-2.0"})
        # Test with mix of invalid and valid licenses
        result = dependency.only_opensource_licenses("InvalidLicense, MPL-2.0")
        self.assertEqual(result, ["MPL-2.0"])
