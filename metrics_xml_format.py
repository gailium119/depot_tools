...
def GetMetricsDir(path):
    ...
    normalized_abs_dirname = os.path.dirname(os.path.abspath(path)).replace(
        os.sep, '/')
    print(f"GetMetricsDir({path}): %s %s %s\r\n" %
          (os.path.abspath(path), os.path.dirname(
              os.path.abspath(path)), normalized_abs_dirname))
    for xml_dir in metrics_xml_dirs:
        result = normalized_abs_dirname.endswith(xml_dir)
        print(f"\r\nxml_dir: {xml_dir}: {result}")
        if normalized_abs_dirname.endswith(xml_dir):
            return xml_dir
    return None


def IsSupportedHistorgramsXML(path):
    supported_xmls = set([
        'histograms.xml',
        'enums.xml',
...
def FindMetricsXMLFormatterTool(path, verbose=False):
    ...
    if not os.path.abspath(path).startswith(os.path.abspath(top_dir)):
        log(
            ...
            'skip formatting: %s %s' %
            (os.path.abspath(path), os.path.abspath(top_dir)), verbose)
        return ''

    if xml_dir == 'tools/metrics/histogram':
