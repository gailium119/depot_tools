...
_REVISION_LINE_PATTERN = re.compile(r'^Revision: [a-f0-9]+$', re.MULTILINE)
...
def update_readme_chromium(dependency, roll_to, current_dir):
    ...
    Returns:
        bool: True if update was successful, False if README.chromium wasn't
        found or had multiple dependencies.
    """

    # README.chromium is typically one directory up from the dependency.
    # readme_path = os.path.join(current_dir, 'README.chromium')
    # readme_path = os.path.join(dependency, os.path.pardir, 'README.chromium')

    gclient_root = gclient(['root'])
    readme_path = os.path.normpath(os.path.join(gclient_root, dependency, os.path.pardir, 'README.chromium'))
    if not os.path.isdir(readme_path):
        print('Dependency %s not found at %s' % (dependency, readme_path))
        readme_path = os.path.normpath(
            os.path.join(current_dir, dependency))
        print('Will look for relative dependency at %s' % readme_path)
        if not os.path.isdir(readme_path):
            raise Error('Directory not found: %s (%s)' %
                        (dependency, readme_path))
