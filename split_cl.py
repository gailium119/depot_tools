...
# Writes the listed CLs to the designated file, in a human-readable and editable
# format.
def SaveSplittingToFile(cl_infos: List[CLInfo], filename: str):
    preamble = (
        "# CLs in this file must have the following format:\n"
        "# A 'Reviewers: [...]' line, where '...' is a (possibly empty) list "
        "of reviewer emails.\n"
        "# A 'Directories: ...' line, where '...' is any string (by default, "
...
# Record the splitting in case the user wants to modify and/or re-use it
def SaveSplittingToTempFile(cl_infos: List[CLInfo]):
    ...
    SaveSplittingToFile(cl_infos, temp_name)
...
# Matches 'Directories: ...', extracts the ...
directories_re = re.compile(r'Directories:\s*(.+)')
...
def ParseSplittings(lines: List[str]) -> List[CLInfo]:
    ...
    - A 'Directories: ' line containing anything, and
    ...
    for line in lines:
        ...
        # Directories is just used as a description, so any string is fine
        m = re.fullmatch(directories_re, line)
        if m:
