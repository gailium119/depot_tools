...
class CMDFormatTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        mock.patch('cl_format._RunCommand').start()
    ...
    def testClangFormatDiffFull(self):
        ...
        cl_format.settings.GetFormatFullByDefault.return_value = False
        diff_file = [os.path.join(self._top_dir, 'test.cc')]
        mock_opts = mock.Mock(full=True, dry_run=True, diff=False)

        # Diff
        cl_format._RunCommand.side_effect = self._run_command_mock('  // test')
    ...
    def testClangFormatDiff(self):
        ...
        # Diff
        cl_format._RunCommand.side_effect = self._run_command_mock('error')
        return_value = cl_format._RunClangFormatDiff(mock_opts, ['.'],
                                                  self._top_dir, 'HEAD')
        self.assertEqual(2, return_value)

        # No diff
        cl_format._RunCommand.side_effect = self._run_command_mock('')
        return_value = cl_format._RunClangFormatDiff(mock_opts, ['.'],
                                                  self._top_dir, 'HEAD')
        self.assertEqual(0, return_value)
    ...
    def testRunMetricsXMLFormatSkipIfPresubmit(self, find_top_dir):
        ...
        return_value = cl_format._RunMetricsXMLFormat(mock_opts, files,
                                                   self._top_dir, 'HEAD')
        cl_format._RunCommand.assert_not_called()
        self.assertEqual(0, return_value)
    ...
    def testRunMetricsFormatWithUkm(self, find_top_dir):
        ...
        cl_format._RunCommand.assert_called_with([
            mock.ANY,
    ...
    def testRunMetricsFormatWithHistograms(self, find_top_dir):
        ...
        cl_format._RunCommand.assert_has_calls([
            mock.call(
    ...
    def testLUCICfgFormatWorks(self, mock_call):
        ...
        ret = cl_format._RunLUCICfgFormat(mock_opts, files, self._top_dir, 'HEAD')
