        ...
        self._commit_git(
            'repo_1', {
                'DEPS': create_deps_content(self.git_base, {
                    ...
                    'src/third_party/with_divider/src': repo2_revision
                }),
                'README.chromium': '\n'.join([
                    'Name: test repo',
                ...
                'third_party/with_divider/README.chromium': '\n'.join([
...
class RollDepTest(fake_repos.FakeReposTestBase):
    ...
    def setUp(self):
        ...
        self.all_repos = [
            ...
            'src/third_party/with_divider/src',
        ]
        if self.enabled:
            self.call([
    ...
    def assert_deps_match(self, expected_path_to_revision_map):
        ...
        for path, revision in expected_path_to_revision_map.items():
            path_dir = os.path.join(self.root_dir, path)
            self.assertEqual(self.gitrevparse(path_dir), revision)
        expected_content = create_deps_content(self.git_base,expected_path_to_revision_map)
    ...
    def testRollsDepWithReadme(self):
        ...
        self.assert_deps_match({
            p: latest_revision for p in self.all_repos
        })
        self.assertEqual(returncode, 0)
        for path in self.all_repos:
            ...
            if os.path.exists(readme_path):
                with open(readme_path, 'r') as f:
                    contents = f.read()
            if path == 'src/third_party/with_divider/src':
                self.assertIn('DEPENDENCY DIVIDER', contents)
    ...
    def testRollsDepToSpecificRevision(self):
        ...
        self.assert_deps_match({
            ...
            'src/third_party/repo_2/src': self.githash('repo_2', 1),
            'src/third_party/repo_2B/src': self.githash('repo_2', 1),
            'src/third_party/with_divider/src': self.githash('repo_2', 1)
        })
