...
def SetupJunction(toolchain_dir):
    ...
    if sys.platform.startswith('win32'):
        print('Setup a directry junction %s to %s' %
              (junction_dir, toolchain_dir))
        subprocess.run(['mklink', '/J', junkction_dir, toolchain_dir],
                       shell=True,
                       check=True)
    else:
        print('Setup a symlink %s to %s' % (junction_dir, toolchain_dir))
        os.symlink(toolchain_dir, junction_dir)
    return junction_dir
